...
export type NewFolderScreenProps = StackScreenProps<
  HomeStackParamList,
  'NewFolder'
>;
...
const NewFolderScreen = ({navigation, route}: NewFolderScreenProps) => {
  ...
  const onSubmitName = async () => {
    try {
      ...
      const response = await API.post('/folders', requestBody, {
        headers: {
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <View
          ...
          <Pressable onPress={() => navigation.navigate('FolderList', {newFolder: null})}>
            <Text
              style={{
                fontSize: 17,
                fontWeight: '400',
